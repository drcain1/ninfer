...
public:
    ...
    std::size_t read_direct(std::uint64_t absolute_offset, std::span<std::byte> destination) const {
        ...
        while (total < destination.size()) {
            constexpr std::size_t maximum_read = 1ULL << 30;
            const DWORD amount =
                static_cast<DWORD>(std::min(maximum_read, destination.size() - total));
            ...
            if (!started) {
                ...
                if (error != ERROR_IO_PENDING ||
                    ...
                    const DWORD final_error = error == ERROR_IO_PENDING ? ::GetLastError() : error;
